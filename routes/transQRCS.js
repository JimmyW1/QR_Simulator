/**
 * Created by CuncheWang on 2018/12/10.
 */

var aes = require('../common/aes');
var db = require('../data/db');

exports.doPostProcessing = function (reqJson, res, next) {
    console.log("=============QRCS processing=================");
    var action = reqJson.action;
    console.log("action=" + action);

    if (action === 'register') {
        register(reqJson, res, next);
    } else if (action === 'generate') {
        generate(reqJson, res, next);
    } else if (action === 'inquiry') {
        inquiry(reqJson, res, next);
    }
};

function register(reqJson, res, next) {
    console.log("=============QRCS register=================");
    var data = reqJson.data;

    var terminalType = data.terminalType;
    var mid = data.mid;
    var tid = data.tid;
    var partnerCode = data.partnerCode;

    console.log("terminalType=" + terminalType);
    console.log("mid=" + mid);
    console.log("tid=" + tid);
    console.log("partnerCode=" + partnerCode);

    // TODO check mid tid and so on.
    var responsePlainData = {
        "code":"0000",
        "message":"success",
        "data":{
            "mid": "",
            "tid":""
        }
    };
    responsePlainData.mid = mid;
    responsePlainData.tid = tid;

    console.log("responsePlainData=[%s]", JSON.stringify(responsePlainData));
    var tidTwk = db.getPlainTwkByTid(tid);
    console.log("tidTwk=" + tidTwk);
    var responseEncryptData = aes.AesZeroPaddingEncryptFunc(JSON.stringify(responsePlainData), tidTwk, "00000000000000000000000000000000");
    var index = db.getEncryptedTwkIndexByTid(tid);

    var responseJson = {"key_index": "", "data": ""};
    responseJson.key_index = index;
    responseJson.data = responseEncryptData;
    res.send(responseJson);
}

function generate(reqJson, res, next) {
    console.log("=============QRCS generate=================");
    var data = reqJson.data;

    var mid = data.mid;
    var tid = data.tid;
    var amount = data.amount;
    var invoice = data.invoice;

    console.log("mid=" + mid);
    console.log("tid=" + tid);
    console.log("amount=" + amount);
    console.log("invoice=" + invoice);

    // TODO check mid tid and so on.
    var responsePlainData = {
        "code":"0000",
        "message":"success",
        "data":{
            "responseCode":"",
            "qrCodeType":"",
            "qrCodeID":"",
            "poi":"",
            "qrCode":"",
            "rawQRCode":"",
            "expiryTime":"",
            "amount":"",
            "currencyCode":"",
            "currencyName":"",
            "invoice":"",
            "merchantID":"",
            "merchantName":"",
            "terminalID":"",
            "terminalName":"",
            "channels":[
                {
                    "channelCode":"MTC",
                    "channelName":"MasterCard",
                    "seqNo":"1"
                },
                {
                    "channelCode":"VSA",
                    "channelName":"VISA",
                    "seqNo":"2"
                }
            ]
        }
    };
    responsePlainData.data.responseCode = "000";
    responsePlainData.data.qrCodeType = "EM";
    responsePlainData.data.qrCodeID = "123456";
    responsePlainData.data.poi = "12";
    responsePlainData.data.qrCode = "";
    responsePlainData.data.rawQRCode = "rawQRCode";
    responsePlainData.data.expiryTime = "2018-08-14 09:52:39";
    responsePlainData.data.amount = "12.3";
    responsePlainData.data.currencyCode = "764";
    responsePlainData.data.currencyName = "Baht";
    responsePlainData.data.invoice = "123456";
    responsePlainData.data.merchantID = "1234567890";
    responsePlainData.data.merchantName = "TEST";
    responsePlainData.data.terminalID = tid;
    responsePlainData.data.terminalName = "TEST";

    console.log("responsePlainData=[%s]", JSON.stringify(responsePlainData));
    var tidTwk = db.getPlainTwkByTid(tid);
    console.log("tidTwk=" + tidTwk);
    var responseEncryptData = aes.AesZeroPaddingEncryptFunc(JSON.stringify(responsePlainData), tidTwk, "00000000000000000000000000000000");
    var index = db.getEncryptedTwkIndexByTid(tid);

    var responseJson = {"key_index": "", "data": ""};
    responseJson.key_index = index;
    responseJson.data = responseEncryptData;
    res.send(responseJson);
}

function inquiry(reqJson, res, next) {
    console.log("=============QRCS inquiry=================");
    var data = reqJson.data;

    var mid = data.mid;
    var tid = data.tid;
    var qrid = data.qrid;

    console.log("mid=" + mid);
    console.log("tid=" + tid);
    console.log("qrid=" + qrid);

    // TODO check mid tid and so on.
    var responsePlainData = {
        "code":"0000",
        "message":"success",
        "data":[{
            "amount":"",
            "transactionId":"",
            "fastEasySlipNumber":"",
            "transactionDateandTime":"",
            "thaiQRTag":"",
            "merchantPAN":"",
            "consumerPAN":"",
            "currencyCode":"",
            "terminalId":"",
            "qrId":"",
            "merchantId":"",
            "traceNo":"",
            "authorizeCode":"",
            "paymentMethod":"",
            "transactionType":"",
            "typeOfCard":""
        }]
    };
    responsePlainData.data[0].amount = "1.0";
    responsePlainData.data[0].transactionId = "1234567890";
    responsePlainData.data[0].fastEasySlipNumber = "123456";
    responsePlainData.data[0].transactionDateandTime = "2018-06-28T10:18:29.664+07:00";
    responsePlainData.data[0].thaiQRTag = "image_base64";
    responsePlainData.data[0].merchantPAN = "3203124558693101";
    responsePlainData.data[0].consumerPAN = "4307-12XX-XXXX-0402";
    responsePlainData.data[0].currencyCode = "764";
    responsePlainData.data[0].terminalId = "1234567890";
    responsePlainData.data[0].qrId = "1234567890";
    responsePlainData.data[0].merchantId = "1234567890";
    responsePlainData.data[0].traceNo = "123456";
    responsePlainData.data[0].authorizeCode = "123456";
    responsePlainData.data[0].paymentMethod = "QRCS";
    responsePlainData.data[0].transactionType = "AUTH";
    responsePlainData.data[0].typeOfCard = "VISA";

    console.log("responsePlainData=[%s]", JSON.stringify(responsePlainData));
    var tidTwk = db.getPlainTwkByTid(tid);
    console.log("tidTwk=" + tidTwk);
    var responseEncryptData = aes.AesZeroPaddingEncryptFunc(JSON.stringify(responsePlainData), tidTwk, "00000000000000000000000000000000");
    var index = db.getEncryptedTwkIndexByTid(tid);

    var responseJson = {"key_index": "", "data": ""};
    responseJson.key_index = index;
    responseJson.data = responseEncryptData;
    res.send(responseJson);
}
